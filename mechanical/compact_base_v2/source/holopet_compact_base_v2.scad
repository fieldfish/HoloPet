// HoloPet 紧凑底座 V2 — 带实物部件代理模型
// 2026-09-18, unit: mm
// `part` selects one printable part or a reference assembly.
// OpenSCAD example:
//   openscad.com -o 01_main_shell.stl -D "part=\"main_shell\"" holopet_compact_base_v2.scad
...
audio_keepout_l = 75.0;        // module + adapter/cable reserve
audio_body_l = 56.0;
audio_w = 28.0;
audio_t = 12.0;
...
module main_shell() {
    difference() {
        ...
        // Pi USB/RJ45 combined window on +X, large enough for plugs.
        radial_window(0,hardware_low_z-1,64,30,9);
        // USB-C / HDMI cable service window between speakers.
        radial_window(315,27,30,18,9);
...
module audio_component_carrier() {
    difference() {
        union() {
            // flat 75x28 keepout tray for USB sound card + adapter
            rounded_box(78,32,3,3);
            // low side lips; connector ends remain open
            translate([-39,-16,3]) cube([78,2,8]);
            translate([-39,14,3]) cube([78,2,8]);
            speaker_frame(1);
            speaker_frame(-1);
        ...
        for(x=[-25,25]) translate([x,-18,-eps]) cube([4,36,3+2*eps]);
...
module audio_model() {
    ...
    color([0.25,0.25,0.28]) translate([-audio_keepout_l/2,-8,audio_floor_z+3]) cube([audio_keepout_l,16,6]);
}
